...
class HacsRepository:
    ...
    async def async_get_legacy_repository_object(
        ...
        try:
            self.logger.debug("Fetching repository: %s (private repos supported if token has 'repo' scope)", self.data.full_name)
            repository = await self.hacs.github.get_repo(self.data.full_name, etag)
            self.logger.info("Successfully fetched repository: %s (private: %s)", self.data.full_name, repository.private)
            ...
            self.logger.error("Failed to fetch repository %s: %s (check if token has 'repo' scope for private repos)", self.data.full_name, exception)
