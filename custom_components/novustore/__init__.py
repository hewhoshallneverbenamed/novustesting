...
async def _async_initialize_integration(
    ...
    # Log token status (without exposing the token itself)
    token_status = "configured" if hacs.configuration.token else "NOT CONFIGURED"
    hacs.log.info("GitHub token status: %s", token_status)
    if hacs.configuration.token:
        hacs.log.info("GitHub API client initialized - private repositories should be accessible")
