...
class HacsBase:
    ...
    def set_active_categories(self) -> None:
        ...
        self.enable_hacs_category(HacsCategory.INTEGRATION)
    ...
    async def async_get_all_category_repositories(self, _=None) -> None:
        ...
        await self.async_get_category_repositories_experimental("integration")
    ...
    async def async_setup_frontend_endpoint_plugin(self) -> None:
        ...
        www_community_path = self.hass.config.path("www/community")
        www_exists = await async_exists(self.hass, www_community_path)
        
        self.log.info("Plugin endpoint check:")
        self.log.info("  - Already active: %s", self.status.active_frontend_endpoint_plugin)
        self.log.info("  - www/community path: %s", www_community_path)
        self.log.info("  - www/community exists: %s", www_exists)
        
        if self.status.active_frontend_endpoint_plugin or not www_exists:
            self.log.info("Skipping plugin endpoint setup")
            return

        self.log.info("Setting up plugin endpoint")
        use_cache = self.core.lovelace_mode == "storage"
        self.log.info(
            "<NovuStoreFrontend> %s mode, cache for %s: %s",
        ...
        self.log.info("Plugin endpoint registered at: %s", URL_BASE)
